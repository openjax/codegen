...
public enum RadixTree {
  ...
  public static RadixTree findNext(final RadixTree previous, final int position, final char ch) {
    if (position == 0) {
      final int index = org.openjax.standard.codegen.radixtree.RadixTreeEnumUtil.binarySearch(RadixTree.values(), RadixTree.root, ch, position);
      return index < 0 ? null : RadixTree.values()[index];
    }

    if (position <= previous.tree.length) {
      final int[] tree = previous.tree[position - 1];
      final int index = org.openjax.standard.codegen.radixtree.RadixTreeEnumUtil.binarySearch(RadixTree.values(), tree, ch, position);
